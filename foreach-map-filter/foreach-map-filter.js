...
function showFirstAndLast(arr){
    let resultArray = [];
    arr.forEach(function(value) {
        resultArray.push(value[0] + "" + value[value.length - 1]);
    });
    return resultArray;
}
    ...
    for(let char of str){
        ...
        if(isVowel(charLower)){
            for(let letter of str){
                if(letter.toLowerCase() == charLower){
                    vowelCount++;
                }
            }
        }
        if(vowelCount > 0){
            vowelsResult[charLower] = vowelCount;
        }
    }
    return vowelsResult;
}
...
function removeVowels(str){
    ...
    for(let letter of str){
        if(!isVowel(letter.toLowerCase())){
            strResult = strResult.concat('', letter.toLowerCase());
        }
    }
    return strResult;
}
